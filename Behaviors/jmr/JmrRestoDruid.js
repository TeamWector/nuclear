...
export class JmrRestoDruidBehavior extends Behavior {
  ...
  getLifebloomTarget() {
    ...
    availableTargets.forEach(friend => {
      // Use hasVisibleAura to check for active Lifebloom buff
      if (friend.hasVisibleAura(auras.lifebloomResto)) {
    ...
    const dpsWithoutLifebloom = heal.friends.DPS.filter(friend => 
      ...
      me.distanceTo(friend) <= 40 &&
      !friend.hasVisibleAura(auras.lifebloomResto)
    );
    ...
    // 3. Me (only if we don't have Lifebloom)
    if (!me.hasVisibleAura(auras.lifebloomResto)) {
      return me;
    }
    ...
    if (Settings.PrioritizeLifebloomOnTanks) {
      const tank = heal.friends.Tanks.find(tank => 
        ...
        !tank.hasVisibleAura(auras.lifebloomResto)
      );
      if (tank) return tank;
    }
  ...
  getLifebloomHealingTarget() {
    ...
    const currentLifeblooms = heal.friends.All.filter(friend => 
      friend && !friend.deadOrGhost && friend.hasVisibleAura(auras.lifebloomResto)
    ).length;
  ...
  getRampLifebloomTarget() {
    ...
    const currentLifeblooms = allTargetsIncludingMe.filter(friend => {
      try {
        return friend && !friend.deadOrGhost && friend.hasVisibleAura(auras.lifebloomResto);
      } catch (error) {
        return false;
      }
    }).length;
    ...
    if (me.hasAura(auras.undergrowth) && maxLifeblooms === 2) {
      ...
      if (friendsWithLifebloom.length >= 1 && !me.hasVisibleAura(auras.lifebloomResto)) {
  ...
  hasActiveHoTs() {
    ...
    const friendsWithHoTs = heal.friends.All.filter(friend => 
      friend && !friend.deadOrGhost &&
      (friend.hasAura(auras.lifebloom) || 
       friend.hasAura(auras.rejuvenation) ||
       friend.hasAura(auras.cenarionWard) ||
